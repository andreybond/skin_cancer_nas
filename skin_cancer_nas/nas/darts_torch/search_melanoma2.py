...
def run(args):
    ...
    os.makedirs('/mnt/models/multi_search_8Dec2020_128x128_12ch/'+checkpoints_folder, exist_ok=True)
    ...
    logging.basicConfig(filename='/mnt/models/multi_search_8Dec2020_128x128_12ch/'+checkpoints_folder+'/search_log.txt',
                        filemode='a',  # a - append
    ...
    trainer = DartsTrainer(cnn_model,
                        ...
                        #    metrics=lambda output, target: f1_score(output.cpu().detach().numpy(), target.cpu().detach().numpy()),
                           ...
                           workers=0,
                           callbacks=[LRSchedulerCallback(lr_scheduler), ArchitectureCheckpoint("/mnt/models/multi_search_8Dec2020_128x128_12ch/"+checkpoints_folder)])
...
if __name__ == "__main__":
    ...
    arch_channels = 12
    runs_args = [
        SimpleNamespace(layers=1, batch_size=1, log_frequency=10, epochs=50, channels=arch_channels, unrolled=False, visualization=do_visualize, \
                        checkpoints_folder='search_1lr_4ch_5classes', cuda_id=cuda_id_, \
                        valid_channels=CHANNELS, \
                        classes_set=CLASSES_SET),

        SimpleNamespace(layers=2, batch_size=1, log_frequency=10, epochs=50, channels=arch_channels, unrolled=False, visualization=do_visualize, \
                        checkpoints_folder='search_2lr_4ch_5classes', cuda_id=cuda_id_, \
                        valid_channels=CHANNELS, \
                        classes_set=CLASSES_SET),
        
        SimpleNamespace(layers=3, batch_size=1, log_frequency=10, epochs=50, channels=8, unrolled=False, visualization=do_visualize, \
                        checkpoints_folder='search_3lr_4ch_5classes', cuda_id=cuda_id_, \
                        valid_channels=CHANNELS, \
                        classes_set=CLASSES_SET),

        SimpleNamespace(layers=4, batch_size=1, log_frequency=10, epochs=50, channels=arch_channels, unrolled=False, visualization=do_visualize, \
                        checkpoints_folder='search_4lr_4ch_5classes', cuda_id=cuda_id_, \
                        valid_channels=CHANNELS, \
                        classes_set=CLASSES_SET),

        SimpleNamespace(layers=5, batch_size=1, log_frequency=10, epochs=50, channels=arch_channels, unrolled=False, visualization=do_visualize, \
                        checkpoints_folder='search_5lr_4ch_5classes', cuda_id=cuda_id_, \
                        valid_channels=CHANNELS, \
                        classes_set=CLASSES_SET)#,

        # SimpleNamespace(layers=6, batch_size=4, log_frequency=10, epochs=50, channels=arch_channels, unrolled=False, visualization=do_visualize, \
        #                 checkpoints_folder='search_6lr_7ch_3classes', cuda_id=cuda_id_, \
        #                 valid_channels=CHANNELS, \
        #                 classes_set=CLASSES_SET),

        # SimpleNamespace(layers=7, batch_size=4, log_frequency=10, epochs=50, channels=arch_channels, unrolled=False, visualization=do_visualize, \
        #                 checkpoints_folder='search_7lr_7ch_3classes', cuda_id=cuda_id_, \
        #                 valid_channels=CHANNELS, \
        #                 classes_set=CLASSES_SET)
    ]
