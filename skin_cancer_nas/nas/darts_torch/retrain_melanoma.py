...
def main(args: ArgumentParser) -> None:
    ...
    #optimizer = torch.optim.Adam(model.parameters(), lr=0.001, betas=(0.9, 0.999), eps=1e-08, weight_decay=3.0E-4, amsgrad=False)
...
if __name__ == "__main__":
    ...
    # X series - my experiments, not requested by Ilze
    # CLASSES_SET_X1 = [ \
    #    ClassesDefinition(root_folders_list=ROOT_PATHS, diagnoses_names_list=['c44'], class_name='NonMelanomaCancer', int_label=0), \
    #    ClassesDefinition(root_folders_list=ROOT_PATHS, diagnoses_names_list=['d22'], class_name='Nevuss', int_label=1) \
    # ]
    ...
    for L in layers_list:
        runs_args.append(
            SimpleNamespace(layers=L, batch_size=batch_size, log_frequency=10, epochs=epochs, channels=16, unrolled=False, \
                            ...
                            cuda_id=cuda_id_, \
                            target_folder_name='/mnt/models/darts_retrained/6ch_128x128_no_metainfo_registered/XV2_SGD_orig_02DropChannel_{}lrs_2oct_ClassSet5_ManCorected_registered'.format(L), \
                            valid_channels=CHANNELS, \
                            channels_to_zero_out=channels_to_zero_out, \
                            channel_drop_prob=0.2, \
                            classes_set=CLASSES_SET_5),
        )
