...
from nni.nas.pytorch.proxylessnas.mutator import ProxylessNasMutator
from nni.nas.pytorch.proxylessnas.utils import cross_entropy_with_label_smoothing, accuracy

logger = logging.getLogger(__name__)

class ProxylessNasTrainerMelanoma(BaseTrainer):
            ...
            for i, (images, labels) in enumerate(self.valid_loader):
                ...
                if i % 10 == 0 or i + 1 == len(self.valid_loader):
                    test_log = 'Valid' + ': [{0}/{1}]\t'\
                                        ...
                                        'Top-1 acc {top1_val:.3f} ({top1_avg:.3f})'.\
                        format(i, len(self.valid_loader) - 1, \
            ...
            for i, (images, labels) in enumerate(data_loader):
                ...
                if i % 10 == 0 or i + 1 == nBatch:
                    batch_log = 'Warmup Train [{0}][{1}/{2}]\t' \
                                ...
                                'Loss {losses_val:.4f} ({losses_avg:.4f})\t' \
                                'Top-1 acc {top1_val:.3f} ({top1_avg:.3f})\t' \
                                'lr {lr:.5f}'. \
                        format( epoch + 1, i, nBatch - 1, 
                                batch_time_val=batch_time.val, batch_time_avg=batch_time.avg,
                                data_time_val=data_time.val, data_time_avg=data_time.avg, 
                                losses_val=losses.val.item(), losses_avg=losses.avg.item(), 
                                top1_val=top1.val.item(), top1_avg=top1.avg.item(),
                                lr=warmup_lr)
                    logger.info(batch_log)
            val_loss, val_top1 = self._validate()
            val_log = 'Warmup Valid [{0}/{1}]\tloss {2:.3f}\ttop-1 acc {3:.3f}\t' \
                      'Train top-1 {top1_avg:.3f}'. \
                format(epoch + 1, self.warmup_epochs, val_loss.item(), val_top1.item(), top1_avg=top1.avg.item())
            logger.info(val_log)
    ...
    def _train(self):
        ...
        for epoch in range(self.train_curr_epoch, self.n_epochs):
            ...
            if (epoch + 1) % self.arch_valid_frequency == 0:
                val_loss, val_top1 = self._validate()
                val_log = 'Valid [{0}]\tloss {1:.3f}\ttop-1 acc {2:.3f}t' \
                          'Train top-1 {top1_avg:.3f}'. \
                    format(epoch + 1, val_loss.item(), val_top1.item(), top1_avg=top1.avg.item())
                logger.info(val_log)
            self.save_checkpoint()
