...
def get_data_three_channels(ch1, ch2, ch3):
    d = data[:, :, :, [int(ch1), int(ch2), int(ch3)]]
...
def objective(args, f_log_metrics):
    ch1 = args['ch1']
    ch2 = args['ch2']
    ch3 = args['ch3']
    print('-+-===objective(123):',ch1, ch2, ch3)
    ...
    clf.compile(optimizer=adam, loss='binary_crossentropy', metrics=[f1])
    
    log_callback = callbacks.LambdaCallback(on_epoch_end=lambda _, logs: f_log_metrics(logs=logs))

    history = clf.fit(X_train_aug, y_train_aug, batch_size=32, epochs=75, 
                        validation_data=(X_test_aug, y_test_aug), verbose=0, class_weight={1:10, 0:1}, callbacks=[log_callback])

    # history = clf.fit(X_train_aug, y_train_aug) #, max_trials=25) #time_limit=12 * 60 * 60)
    # history2 = clf.final_fit(X_train_aug, y_train_aug, X_test_aug, y_test_aug, retrain=True)
    y = clf.evaluate(X_test_aug, y_test_aug)
    print(y)
...
#     print(history.history['val_f1'])
    ...
    # return {'history': None, 'channels': str([ch1] + [ch2] + [ch3]) }
